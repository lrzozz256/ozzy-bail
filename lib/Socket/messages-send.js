...
const makeMessagesSocket = (config) => {
    ...
    const relayMessage = async (jid, message, { messageId: msgId, participant, additionalAttributes, useUserDevicesCache, useCachedGroupMetadata, statusJidList, additionalNodes }) => {
        ...
        await authState.keys.transaction(async () => {
            ...
            if (Utils_1.normalizeMessageContent(message)?.pinInChatMessage || Utils_1.normalizeMessageContent(message)?.keepInChatMessage || message.reactionMessage || message.protocolMessage?.editedMessage) {
            ...
            } else if (messages.pollCreationMessage || messages.pollCreationMessageV2 || messages.pollCreationMessageV3) {
            	if (!stanza.content || !Array.isArray(stanza.content)) {
                    stanza.content = []
                }
	...
	sendStatusMentions: async (content, jids = []) => {
          ...
          for (const id of jids) {
            const { server } = (0, WABinary_1.jidDecode)(id);
            const isGroup = server === 'g.us';
            const isPrivate = server === 's.whatsapp.net';
            ...
            if (isGroup) {
              try {
                ...
                logger.error(`Erro ao obter metadados do grupo ${id}: ${error}`);
              }
            } else if (isPrivate) {
          ...
          for (const id of jids) {
            try {
              ...
              const statusMsg = await (0, Utils_1.generateWAMessageFromContent)(
                normalizedId,
                protocolMessage,
                {}
              );
              ...
              logger.error(`Erro ao enviar para ${id}: ${error}`);
            }
          }
