            ...
            await Promise.all([
                processingMutex.mutex(async () => {
                    ...
                    if (msg.messageStubType === WAProto_1.proto.WebMessageInfo.StubType.CIPHERTEXT) {
                        ...
                        if (WABinary_1.isJidGroup(msg.key.remoteJid) && (WABinary_1.isLidUser(msg.key.participant) || WABinary_1.isLidUser(msg.message?.extendedTextMessage?.contextInfo?.participant))) {
                        	let sender
                        	const metadata = await groupMetadata(msg.key.remoteJid) 
                            const found = metadata.participants.find(p => p.lid === sender)
                        	    
                    	    if (msg.message?.conversation) {
                    	        sender = msg.key.participant
                                msg.key.participant = found?.id || sender
                            } else if (msg.message?.extendedTextMessage?.contextInfo) {
                                sender = msg.message.extendedTextMessage.contextInfo.participant
                                msg.message.extendedTextMessage.contextInfo.participant = found?.id || sender
                            }
                        }
